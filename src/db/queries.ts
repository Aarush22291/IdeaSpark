...
export async function getActiveTracks() {
  return db.select().from(tracks).where(eq(tracks.isActive, true)).orderBy(tracks.name);
}
...
export async function getTeamByLeader(leadUserId: string) {
  const [team] = await db.select().from(teams).where(eq(teams.leadUserId, leadUserId)).limit(1);
  return team ?? null;
}
...
export async function getTeamDashboard(teamId: string) {
  ...
  const [teamMembers, submission] = await Promise.all([
    db.select().from(members).where(eq(members.teamId, teamId)).orderBy(members.name),
    db.select().from(submissions).where(eq(submissions.teamId, teamId)).limit(1),
  ]);
...
export async function getEventConfig() {
  const [config] = await db.select().from(eventConfig).where(eq(eventConfig.id, 1)).limit(1);
  return config ?? null;
}
...
export async function listAdminTeams(filters?: {
  ...
  if (filters?.search) conditions.push(ilike(teams.teamName, `%${filters.search}%`));
