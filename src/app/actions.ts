...
async function assertBefore(field: "registrationDeadline" | "submissionDeadline") {
  const [cfg] = await db.select().from(eventConfig).where(eq(eventConfig.id, 1)).limit(1);
...
async function log(actorUserId: string | null, action: string, targetType: string, targetId: string, meta?: unknown) {
...
export async function createTeam(name: string, trackId: string, membersInput: MemberInput[]) {
...
export async function addMember(input: MemberInput) {
  ...
  await log(user.id, "team.member.add", "team", team.id, { memberId: member.id });
  revalidatePath("/dashboard");
  return member;
}
...
export async function removeMember(memberId: string) {
  ...
  await log(user.id, "team.member.remove", "team", team.id, { memberId: member.id });
  revalidatePath("/dashboard");
}
...
export async function setTrack(trackId: string) {
  ...
  const [submission] = await db.select().from(submissions).where(eq(submissions.teamId, team.id)).limit(1);
  if (submission?.status !== "pending_submission") {
    throw new Error("Track cannot be changed after submission review has started");
  }

  const [track] = await db.select().from(tracks).where(and(eq(tracks.id, trackId), eq(tracks.isActive, true))).limit(1);
  if (!track) throw new Error("Invalid or inactive track");

  await db.update(teams).set({ trackId, updatedAt: new Date() }).where(eq(teams.id, team.id));
  revalidatePath("/dashboard");
}

export async function submitSubmission(title: string, description: string, driveLink: string) {
  const user = await requireLead();
  ...
  if (memberCount < MIN_MEMBERS) throw new Error("At least 2 members are required to submit");

  const [existing] = await db.select().from(submissions).where(eq(submissions.teamId, team.id)).limit(1);
  if (!existing) throw new Error("Submission record missing for team");
...
export async function getMyDashboard() {
  ...
  const [teamMembers, submission] = await Promise.all([
    ...
    db.select().from(submissions).where(eq(submissions.teamId, team.id)).limit(1),
  ]);
  return { team, members: teamMembers, submission: submission[0] ?? null };
}
...
export async function addAdmin(email: string, name: string, role: "super_admin" | "evaluator" | "volunteer") {
...
export async function listAdmins() {
  await requireAdminRole(["super_admin"]);
  return db.select({ id: admins.id, name: admins.name, email: admins.email, role: admins.role, createdAt: admins.createdAt })
    .from(admins)
...
export async function updateEventConfig(input: {
  ...
  const [config] = await db.update(eventConfig).set({
    registrationDeadline: input.registrationDeadline,
    submissionDeadline: input.submissionDeadline,
    registrationFee: input.registrationFee,
    updatedAt: new Date(),
  }).where(eq(eventConfig.id, 1)).returning();
...
export async function createEvaluationRound(name: string, description: string | undefined, sequenceNo: number) {
  const admin = await requireAdminRole(["super_admin"]);
  if (!Number.isInteger(sequenceNo) || sequenceNo < 1) throw new Error("Invalid sequence number");
  const [round] = await db.insert(evaluationRounds).values({
    name: name.trim(),
    description: description?.trim() || null,
    sequenceNo,
  }).returning();
...
export async function setActiveEvaluationRound(roundId: string, isActive: boolean) {
  const admin = await requireAdminRole(["super_admin"]);
  const [round] = await db.select().from(evaluationRounds).where(eq(evaluationRounds.id, roundId)).limit(1);
  if (!round) throw new Error("Round not found");

  await db.transaction(async (tx) => {
    if (isActive) {
      await tx.update(evaluationRounds).set({ isActive: false }).where(eq(evaluationRounds.isActive, true));
    }
    await tx.update(evaluationRounds).set({ isActive }).where(eq(evaluationRounds.id, roundId));
  });
...
export async function reviewSubmission(
  ...
  await log(admin.id, `submission.${status}`, "submission", submissionId, { remarks });
  revalidatePath("/admin");
  revalidatePath("/dashboard");
  return submission;
}

export async function setTeamStatus(teamId: string, status: "pending" | "approved" | "rejected") {
  const admin = await requireAdminRole(["super_admin"]);
  const [team] = await db
    .update(teams)
...
export async function upsertScore(roundId: string, teamId: string, score: number, remarks?: string) {
  ...
  const [row] = await db
    .insert(scores)
    .values({ teamId, roundId, evaluatorId: admin.id, score: score.toFixed(2), remarks: remarks?.trim() || null })
    .onConflictDoUpdate({
...
export async function scanAttendance(attendanceCode: string, eventDate?: string) {
  const admin = await requireAdminRole(["volunteer", "super_admin"]);
  const dateValue = eventDate ?? new Date().toISOString().slice(0, 10);

  const [member] = await db.select().from(members).where(eq(members.attendanceCode, attendanceCode)).limit(1);
  if (!member) throw new Error("Invalid attendance code");

  const [row] = await db
    .insert(attendance)
    .values({ memberId: member.id, eventDate: dateValue, scannedBy: admin.id })
    .onConflictDoNothing({ target: [attendance.memberId, attendance.eventDate] })
    .returning();
...
export async function publishResults(enabled: boolean) {
  const admin = await requireAdminRole(["super_admin"]);
  await db.update(eventConfig).set({ resultsPublished: enabled, updatedAt: new Date() }).where(eq(eventConfig.id, 1));
